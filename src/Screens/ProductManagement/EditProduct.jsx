    ...
    const filehandleChange = (event) => {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            reader.onload = (e) => {
                ...
                // Collect the imageDataUrl in an array
                imageDataUrls.push(imageDataUrl);

                // Set the files state with the array of imageDataUrls
                                                ...
                                                
                                                    {
                                                        formData?.product_images && formData?.product_images?.map((item) => (
                                                            <div className="galleryItem col-md-3 mb-3 position-relative" key={item.id}>
                                                                ...
                                                                <img src={item.id ? base_url + item.image : item.image} alt={`Product Image ${item.id}`} />

                                                                {/* Add a button to remove the image */}
                                                                <div className="removeImage" onClick={() => handleRemove(item.id)}>
                                                                    <button type="button">
